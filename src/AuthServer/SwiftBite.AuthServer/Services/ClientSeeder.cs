...
public class ClientSeeder : IHostedService
{
    ...
    // secrets must come from config (env vars/user-secrets), never hardcoded
    private string GetRequiredSecret(string key)
        => _configuration[$"OpenIddictClients:{key}"]
           ?? throw new InvalidOperationException($"Missing required configuration: OpenIddictClients:{key}");
    ...
    
    {
        ...
        if (gatewayClient is null)
        {
            await manager.CreateAsync(new OpenIddictApplicationDescriptor
            {
                ...
                ClientSecret = GetRequiredSecret("GatewaySecret"),
        ...
        if (userServiceClient is null)
        {
            await manager.CreateAsync(new OpenIddictApplicationDescriptor
            {
                ...
                ClientSecret = GetRequiredSecret("UserServiceSecret"),
                ClientType = ClientTypes.Confidential,
        ...
        if (restaurantClient is null)
        {
            await manager.CreateAsync(new OpenIddictApplicationDescriptor
            {
                ...
                ClientSecret = GetRequiredSecret("RestaurantServiceSecret"),
                ClientType = ClientTypes.Confidential,
        ...
        if (orderClient is null)
        {
            await manager.CreateAsync(new OpenIddictApplicationDescriptor
            {
                ...
                ClientSecret = GetRequiredSecret("OrderServiceSecret"),
                ClientType = ClientTypes.Confidential,
        ...
        if (paymentClient is null)
        {
            await manager.CreateAsync(new OpenIddictApplicationDescriptor
            {
                ...
                ClientSecret = GetRequiredSecret("PaymentServiceSecret"),
                ClientType = ClientTypes.Confidential,
        ...
        if (notifClient is null)
        {
            await manager.CreateAsync(
                new OpenIddictApplicationDescriptor
                {
                    ...
                    ClientSecret = GetRequiredSecret("NotificationServiceSecret"),
                    ClientType = ClientTypes.Confidential,
        ...
        if (deliveryClient is null)
        {
            await manager.CreateAsync(new OpenIddictApplicationDescriptor
            {
                ...
                ClientSecret = GetRequiredSecret("DeliveryServiceSecret"),
                ClientType = ClientTypes.Confidential,
