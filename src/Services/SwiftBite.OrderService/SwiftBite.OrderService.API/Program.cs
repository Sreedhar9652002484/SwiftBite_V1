...
builder.Services.AddOpenIddict()
    .AddValidation(options =>
    {
        ...
        var clientSecret = builder.Configuration["OpenIddictClients:OrderServiceSecret"]
            ?? throw new InvalidOperationException("Missing required configuration: OpenIddictClients:OrderServiceSecret");
...
builder.Services.AddHealthChecks()
    .AddSqlServer(
        ...
        name: "SwiftBite_OrderDb")
    .AddKafka(
        config => {
            config.BootstrapServers =
                builder.Configuration["Kafka:BootstrapServers"];
            // e.g. "localhost:9092" or "kafka:29092" inside docker
            SwiftBite.OrderService.Infrastructure.Messaging.KafkaSecurity.Apply(config, builder.Configuration);
        },
        name: "kafka",
        tags: new[] { "messaging" });
