...
public class PaymentsController : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(signature) || !IsValidWebhookSignature(rawBody, signature))
            return BadRequest(new { status = "invalid signature" });

        // event dispatch (order paid/failed/refunded) goes here once needed
        return Ok(new { status = "received" });
    }

    private bool IsValidWebhookSignature(string rawBody, string signature)
    {
